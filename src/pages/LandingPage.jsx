...
const LandingPage = () => {
  ...
  // ✅ FIX 1: Tambahkan state isVideoOpen yang hilang
    ...
    <div className="text-gray-800 antialiased font-poppins selection:bg-[#8477e4] selection:text-white overflow-x-hidden relative">
      <style>{`
        ...
        .color-bubble { 
          ...
          z-index: -1; pointer-events: none !important; animation: float-bubble 15s infinite ease-in-out; 
        }
      ...
      {/* ✅ FIX 2: Modal Video — muncul saat isVideoOpen = true */}
      {isVideoOpen && (
      ...
      <div className="fixed inset-0 overflow-hidden pointer-events-none -z-10">
        <img src="/gambar/bubble.png" className="absolute top-[-10%] left-[-5%] w-[250px] md:w-[400px] opacity-50 animate-bubble-img" alt="" />
        <img src="/gambar/bubble.png" className="absolute top-[40%] right-[-10%] w-[200px] md:w-[300px] opacity-40 animate-bubble-img" style={{ animationDelay: "2s" }} alt="" />
        <img src="/gambar/bubble.png" className="absolute bottom-[-10%] left-[10%] w-[220px] md:w-[350px] opacity-30 animate-bubble-img" style={{ animationDelay: "4s" }} alt="" />
      </div>

      {/* --- NAVBAR --- */}
      <nav className="fixed w-full top-4 md:top-6 z-50 flex justify-center px-4 sm:px-6">
        <div className={`w-full max-w-6xl bg-white/90 backdrop-blur-md shadow-xl px-5 md:px-8 py-3 md:py-4 flex flex-col md:grid md:grid-cols-3 items-center border border-white/50 transition-all duration-300 ${isMenuOpen ? "rounded-3xl" : "rounded-full"}`}>
          <div className="flex w-full md:w-auto justify-between items-center md:justify-start">
            <div className="flex items-center gap-2 md:gap-3">
              ...
              <span className="font-bold text-lg md:text-2xl tracking-tight text-[#1e1b4b]">FinTrack AI</span>
            </div>
            <button onClick={() => setIsMenuOpen(!isMenuOpen)} className="md:hidden text-[#1e1b4b] hover:text-[#8477e4] focus:outline-none p-1">
          ...
          <div className={`${isMenuOpen ? "flex" : "hidden"} md:flex flex-col md:flex-row items-center justify-center w-full mt-4 md:mt-0 font-semibold text-gray-600 text-sm md:text-xs lg:text-sm xl:text-base`}>
            <ul className="flex flex-col md:flex-row gap-4 md:gap-3 lg:gap-6 xl:gap-8 w-full md:w-auto text-center">
              <li><a href="#beranda" onClick={() => setIsMenuOpen(false)} className="block w-full py-1 md:py-0 hover:text-[#8477e4] transition-colors whitespace-nowrap">Home</a></li>
              <li><a href="#demo-video" onClick={() => setIsMenuOpen(false)} className="block w-full py-1 md:py-0 hover:text-[#8477e4] transition-colors whitespace-nowrap">Demo</a></li>
              <li><a href="#tentang" onClick={() => setIsMenuOpen(false)} className="block w-full py-1 md:py-0 hover:text-[#8477e4] transition-colors whitespace-nowrap">Tentang</a></li>
              <li><a href="#fitur" onClick={() => setIsMenuOpen(false)} className="block w-full py-1 md:py-0 hover:text-[#8477e4] transition-colors whitespace-nowrap">Fitur</a></li>
              <li className="hidden md:block"><a href="#dashboard" onClick={() => setIsMenuOpen(false)} className="block w-full py-1 md:py-0 hover:text-[#8477e4] transition-colors whitespace-nowrap">Dashboard</a></li>
              <li><a href="#tim" onClick={() => setIsMenuOpen(false)} className="block w-full py-1 md:py-0 hover:text-[#8477e4] transition-colors whitespace-nowrap">Team</a></li>
              <li><a href="#faq" onClick={() => setIsMenuOpen(false)} className="block w-full py-1 md:py-0 hover:text-[#8477e4] transition-colors whitespace-nowrap">FAQ</a></li>
            </ul>
          </div>

          <div className={`${isMenuOpen ? "flex" : "hidden"} md:flex flex-col md:flex-row items-center md:justify-end w-full mt-4 md:mt-0 pb-2 md:pb-0`}>
            <button
      ...
      {/* --- HERO --- */}
      <section id="beranda" className="min-h-screen flex flex-col justify-center pt-28 pb-16 px-4 sm:px-6 md:px-12 lg:px-20 relative overflow-visible">
        <div className="container mx-auto grid md:grid-cols-2 gap-8 md:gap-10 items-center">

          {/* ✅ FIX 3: Kolom kiri — semua konten termasuk fitur mini ada DI DALAM div ini */}
          <div className="space-y-5 md:space-y-6 z-10 text-center md:text-left">
            ...
            {/* ✅ FIX 4: Fragment ditutup dengan benar, tombol-tombol dalam satu div */}
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-3 sm:gap-4 pt-2">
              ...
              <a
                href="#demo-video"
                className="bg-white/60 backdrop-blur-sm text-[#1e1b4b] text-xs sm:text-sm md:text-base font-bold px-5 py-3.5 sm:px-6 sm:py-4 rounded-full shadow-sm border border-white hover:bg-white transition-colors flex items-center gap-2 sm:gap-3"
              >
            ...
            {/* ✅ FIX 5: Fitur mini berada di dalam kolom kiri (bukan di luar) */}
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 sm:gap-4 pt-4 md:pt-6 text-left">
              <div className="flex items-center gap-2 bg-white/40 backdrop-blur-sm p-2 rounded-2xl border border-white/40 md:bg-transparent md:p-0 md:border-none">
                <div className="w-8 h-8 rounded-full bg-[#f3edff] flex items-center justify-center text-base sm:text-xl shrink-0">✨</div>
                <span className="text-[11px] sm:text-xs font-bold text-[#1e1b4b] leading-tight">Insight AI<br />Real-time</span>
              </div>
              <div className="flex items-center gap-2 bg-white/40 backdrop-blur-sm p-2 rounded-2xl border border-white/40 md:bg-transparent md:p-0 md:border-none">
                <div className="w-8 h-8 rounded-full bg-[#ffe4e6] flex items-center justify-center text-base sm:text-xl shrink-0">🥧</div>
                <span className="text-[11px] sm:text-xs font-bold text-[#1e1b4b] leading-tight">Pengelolaan<br />Budget</span>
              </div>
              <div className="flex items-center gap-2 bg-white/40 backdrop-blur-sm p-2 rounded-2xl border border-white/40 md:bg-transparent md:p-0 md:border-none">
                <div className="w-8 h-8 rounded-full bg-[#fce7f3] flex items-center justify-center text-base sm:text-xl shrink-0">🎯</div>
                <span className="text-[11px] sm:text-xs font-bold text-[#1e1b4b] leading-tight">Target<br />Tabungan</span>
              </div>
              <div className="flex items-center gap-2 bg-white/40 backdrop-blur-sm p-2 rounded-2xl border border-white/40 md:bg-transparent md:p-0 md:border-none">
                <div className="w-8 h-8 rounded-full bg-[#e0f2fe] flex items-center justify-center text-base sm:text-xl shrink-0">📊</div>
                <span className="text-[11px] sm:text-xs font-bold text-[#1e1b4b] leading-tight">Laporan<br />Lengkap</span>
              </div>
            </div>
          </div>

          {/* Kolom kanan: gambar mobile (tampil di bawah teks di HP) */}
          <div className="relative z-10 mt-6 md:mt-0 flex justify-center md:hidden w-full">
            <img src="/gambar/Home.png" alt="Home FinTrack AI" className="w-full max-w-sm sm:max-w-md object-contain drop-shadow-2xl" />
          </div>

          {/* Spacer kosong di Desktop */}
          <div className="hidden md:block w-full h-full pointer-events-none"></div>
        </div>

        {/* Gambar mentok kanan (Desktop only) */}
        <div className="hidden md:block absolute right-0 top-[45%] -translate-y-1/2 w-[44vw] max-w-[700px] lg:max-w-[800px] xl:max-w-[950px] z-10 overflow-visible">
          <img src="/gambar/Home.png" alt="Home FinTrack AI" className="w-full object-contain drop-shadow-2xl scale-105 lg:scale-115 xl:scale-125 origin-right" />
        </div>
          ...
          <div className="bg-white/70 backdrop-blur-xl border border-white/50 rounded-[28px] lg:rounded-full px-6 py-6 lg:px-12 lg:py-6 shadow-[0_10px_40px_-15px_rgba(132,119,228,0.2)] grid grid-cols-2 lg:flex lg:flex-row justify-between items-center gap-6 md:gap-8">
            <div className="flex items-center gap-3 sm:gap-4 w-full lg:w-auto justify-start sm:justify-center">
              <div className="w-11 h-11 sm:w-14 sm:h-14 rounded-full bg-[#f3edff] flex items-center justify-center shadow-[inset_0_2px_8px_rgba(132,119,228,0.1)] shrink-0">
                <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5 sm:w-7 sm:h-7 text-[#8477e4]" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z" />
                </svg>
              </div>
              <div className="flex flex-col">
                <h4 className="font-extrabold text-xl sm:text-2xl text-[#1e1b4b] leading-tight">10K+</h4>
                <p className="text-xs sm:text-sm text-[#8477e4]/80 font-medium mt-0.5">Pengguna Aktif</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 sm:gap-4 w-full lg:w-auto justify-start sm:justify-center">
              ...
              <div className="flex flex-col">
                <h4 className="font-extrabold text-xl sm:text-2xl text-[#1e1b4b] leading-tight">99%</h4>
                <p className="text-xs sm:text-sm text-[#8477e4]/80 font-medium mt-0.5">Transaksi Aman</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 sm:gap-4 w-full lg:w-auto justify-start sm:justify-center">
              ...
              <div className="flex flex-col">
                <h4 className="font-extrabold text-xl sm:text-2xl text-[#1e1b4b] leading-tight">AI</h4>
                <p className="text-xs sm:text-sm text-[#8477e4]/80 font-medium mt-0.5">Analisis Cerdas</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 sm:gap-4 w-full lg:w-auto justify-start sm:justify-center">
              ...
              <div className="flex flex-col">
                <h4 className="font-extrabold text-xl sm:text-2xl text-[#1e1b4b] leading-tight">24/7</h4>
                <p className="text-xs sm:text-sm text-[#8477e4]/80 font-medium mt-0.5">Bantuan AI</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section id="demo-video" className="w-full py-16 px-4 flex flex-col items-center justify-center scroll-mt-24">
        <div className="max-w-4xl w-full text-center mb-8">
          <h2 className="text-2xl sm:text-3xl font-bold text-[#1e1b4b] mb-3">
            Lihat FinTrack AI dalam Aksi
          </h2>
          <p className="text-sm sm:text-base text-gray-500 max-w-xl mx-auto">
            Tonton video demo singkat di bawah ini untuk melihat bagaimana AI kami membantu mengelola keuanganmu secara otomatis.
          </p>
        </div>
        <div className="relative w-full max-w-4xl bg-zinc-950 rounded-2xl overflow-hidden aspect-video shadow-xl border border-gray-200">
          <video
            ref={videoRef}
            src="/video/fintrackaidemo.mp4"
            className="w-full h-full object-contain"
      ...
      {/* --- TENTANG KAMI --- */}
      <section id="tentang" className="py-16 md:py-24 px-4 sm:px-6 md:px-12 lg:px-20 scroll-mt-24">
        <div className="w-full bg-gradient-to-br from-[#8477e4] to-[#e584ee] rounded-[32px] md:rounded-[50px] p-8 sm:p-12 md:p-20 lg:p-24 text-white shadow-3xl relative overflow-hidden flex flex-col lg:flex-row lg:items-center justify-between gap-12">
          <div className="w-full lg:max-w-2xl xl:max-w-3xl space-y-5 md:space-y-8 relative z-10 text-center lg:text-left">
            <h2 className="text-3xl sm:text-5xl md:text-6xl lg:text-7xl font-black leading-tight">
              ...
              <span className="text-white/70 italic text-2xl sm:text-4xl md:text-5xl lg:text-6xl">dengan Insight Cerdas AI</span>
            </h2>
          ...
          <div className="relative lg:absolute lg:right-0 xl:right-10 lg:top-1/2 lg:-translate-y-1/2 scale-100 sm:scale-110 lg:scale-125 xl:scale-140 origin-center lg:origin-right flex justify-center z-10 mt-4 lg:mt-0">
            <img src="/gambar/hpfintrackai.png" alt="FinTrack AI Mobile App" className="w-64 sm:w-80 md:w-96 drop-shadow-2xl" />
          </div>
        </div>
      </section>

      {/* --- FITUR --- */}
      <section id="fitur" className="py-20 md:py-32 px-4 sm:px-6 md:px-12 lg:px-20 scroll-mt-24">
              ...
              <SwiperSlide key={index}>
                <div className="card-fitur-presisi">
                  <div className="w-20 h-20 sm:w-24 sm:h-24 bg-indigo-50 rounded-3xl flex items-center justify-center mb-6 sm:mb-8 shadow-inner shrink-0">
                    <img src={`/gambar/${item.i}`} className="w-12 h-12 sm:w-16 sm:h-16 object-contain" alt={item.t} />
                  </div>
                  <h3 className="text-2xl sm:text-3xl font-bold mb-3 sm:mb-5">{item.t}</h3>
                  <p className="text-gray-500 text-sm sm:text-lg leading-relaxed">{item.d}</p>
                </div>
              </SwiperSlide>
            ))}
          </Swiper>
      ...
      <section id="dashboard" className="hidden md:block py-24 px-6 md:px-20">
        ...
        <div className="flex gap-4 h-[500px] max-w-7xl mx-auto">
          {["berandatampilan.jpeg", "transaksitampilan.jpeg", "budgettampilan.jpeg", "goalstampilan.jpeg", "aitampilan.jpeg"].map((img, idx) => (
            <div
              key={idx}
              ...
              className={`transition-all duration-700 ease-in-out cursor-pointer rounded-[40px] shadow-2xl bg-white overflow-hidden flex items-center justify-center ${
                activeDashboard === idx ? "flex-[4] p-6" : "flex-[1] p-0 opacity-80 hover:opacity-100"
              }`}
            >
              <img
                src={`/gambar/${img}`}
                className={`h-full w-full transition-all duration-700 ${activeDashboard === idx ? "object-contain" : "object-cover object-left"}`}
                alt={`Dashboard ${idx}`}
              />
            </div>
          ))}
      ...
      {/* --- TIM --- */}
      <div id="tim" className="py-16 md:py-24 font-poppins selection:bg-[#8477e4] selection:text-white px-4 sm:px-6 md:px-12 lg:px-20 scroll-mt-24">
        <style>{`
          #tim .swiper { padding: 30px 10px 50px 10px !important; overflow: visible !important; }
          @media (min-width: 768px) { #tim .swiper { padding: 50px 20px !important; } }
          .team-item { position: relative; border-radius: 25px; overflow: hidden; aspect-ratio: 3/4; cursor: grab; transition: all 0.5s ease; box-shadow: 0 20px 40px rgba(0, 0, 0, 0.15); border: 1px solid rgba(0,0,0,0.05); }
          ...
          .team-text-overlay { position: absolute; inset: 0; background: linear-gradient(to top, rgba(0, 0, 0, 0.9) 0%, rgba(0, 0, 0, 0.3) 50%, transparent 100%); display: flex; flex-direction: column; justify-content: flex-end; padding: 20px; }
          @media (min-width: 768px) { .team-text-overlay { padding: 25px; } }
        ...
        <div className="text-center mb-8 md:mb-12">
          <h2 className="text-3xl sm:text-5xl font-bold text-gray-900">Meet Our Team</h2>
        </div>
          ...
          >
            {[...Array(2)].map((_, loopIdx) => (
              <React.Fragment key={loopIdx}>
                {[
                  { n: "Anisa", r: "Back-end Developer", i: "Anisa.png", ig: "https://www.instagram.com/nisaa107_/", li: "https://www.linkedin.com/in/anisa-nisa-534a50287/" },
                  { n: "Shifa Anjani Desha", r: "Front-end Developer", i: "Shifa.jpeg", ig: "https://www.instagram.com/shifaanjanii/", li: "https://www.linkedin.com/in/shifa-anjani-desha-9163683a6/" },
                  { n: "Mohammad El Abror Sholeh", r: "AI Engineer", i: "Abror.jpeg", ig: "https://www.instagram.com/mohamadelabror", li: "https://www.linkedin.com/in/mohamadelabror" },
                  { n: "Hamasah Fazal Aqsha", r: "AI Engineer", i: "Aqso.jpeg", ig: "https://instagram.com/hamaaasah_", li: "https://www.linkedin.com/in/hamasah-fazal-aqsha-b61b86274/" },
                  { n: "Icha Aulia Putri", r: "Data Scientist", i: "Ichaa.png", ig: "https://instagram.com/ichaauliaptrii_", li: "https://www.linkedin.com/in/icha-aulia-putri-253ba5330" },
                  { n: "Nanda Hidayah", r: "Data Scientist", i: "Nanda.jpeg", ig: "https://instagram.com/nndhdyh", li: "https://www.linkedin.com/in/nanda-hidayah-87a7423ab/" },
                ].map((member, index) => (
                  <SwiperSlide key={`${loopIdx}-${index}`}>
                    <div className="team-item">
                      <img src={`/gambar/${member.i}`} alt={member.n} />
                      <div className="team-text-overlay">
                        <h3 className="text-white font-bold text-base sm:text-lg">{member.n}</h3>
                        <p className="text-white/80 text-xs sm:text-sm mb-3">{member.r}</p>
                        <div className="flex gap-2 relative z-20">
                          <a href={member.ig} target="_blank" rel="noopener noreferrer" className="social-icon">
                            <i className="fab fa-instagram"></i>
                          </a>
                          <a href={member.li} target="_blank" rel="noopener noreferrer" className="social-icon">
      ...
      {/* --- FAQ --- */}
      <section id="faq" className="py-16 md:py-32 px-6 scroll-mt-24">
        <div className="max-w-4xl mx-auto">
        <h2 className="text-3xl md:text-5xl font-bold text-center mb-12 md:mb-20 text-gray-900">Any Questions?</h2>
        <div className="space-y-4 md:space-y-6">
          {[
            { q: "Apa itu FinTrack AI?", a: "FinTrack AI adalah platform manajemen keuangan digital pintar yang dirancang untuk membantu kamu mengontrol pengeluaran, mengatur budget otomatis, hingga menetapkan target tabungan. Dilengkapi dengan teknologi Insight Cerdas AI, aplikasi ini akan menganalisis kebiasaan finansialmu dan memberikan rekomendasi terbaik agar keuanganmu tetap sehat dan optimal setiap hari." },
            { q: "Apakah data transaksi saya aman?", a: "Sangat aman. Keamanan privasi dan data transaksimu adalah prioritas utama kami. FinTrack AI menggunakan sistem keamanan berlapis dan enkripsi standar industri untuk memastikan bahwa seluruh riwayat transaksi, catatan saldo, dan data pribadi kamu tersimpan dengan aman serta tidak akan disalahgunakan oleh pihak mana pun." },
            { q: "Berapa biaya langganannya?", a: "FinTrack AI bisa kamu gunakan secara Gratis untuk fitur-fitur dasar seperti pencatatan transaksi harian dan pembuatan budget sederhana. Namun, untuk menikmati fitur premium tanpa batas seperti analisis mendalam dari Insight AI, laporan keuangan kustom, dan pelacakan target tabungan tingkat lanjut kami menyediakan paket langganan premium dengan harga yang sangat ramah di kantong." },
            { q: "Apakah bisa digunakan di Mobile?", a: "Tentu saja bisa! FinTrack AI mengusung desain yang sepenuhnya responsive. Kamu bisa mengakses dan menggunakannya dengan sangat nyaman melalui browser di smartphone (HP) kamu tanpa kehilangan keindahan tampilan maupun kelengkapan fiturnya, sama lancarnya seperti saat diakses lewat laptop atau desktop." },
          ].map((item, i) => (
            <div key={i} className={`bg-white rounded-3xl p-5 md:p-6 shadow-sm border transition-all duration-300 ${activeFaq === i ? "border-[#8477e4]" : "border-gray-100"}`}>
              <button className="w-full flex justify-between items-center text-left py-2" onClick={() => setActiveFaq(activeFaq === i ? null : i)}>
                <span className="text-lg md:text-xl font-bold text-gray-800 pr-4">{item.q}</span>
                <i className={`fas fa-chevron-down transition-transform duration-300 ${activeFaq === i ? "rotate-180 text-[#8477e4]" : "text-gray-400"}`}></i>
              </button>
              <div className={`overflow-hidden transition-all duration-300 ease-in-out ${activeFaq === i ? "max-h-96 opacity-100 mt-4" : "max-h-0 opacity-0"}`}>
                <p className="text-gray-500 text-base md:text-lg leading-relaxed">{item.a}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-12 justify-items-center">
          <div className="flex flex-col items-center space-y-4 max-w-xs">
            <div className="flex items-center gap-3">
              <img src="/gambar/logo.png" className="w-10" alt="FinTrack AI Logo" />
              <span className="font-bold text-2xl">FinTrack AI</span>
            </div>
            <p className="text-gray-500 leading-relaxed text-sm">Solusi manajemen keuangan modern untuk generasi masa kini. Cerdas, efisien, dan transparan.</p>
          </div>
          <div className="flex flex-col items-center">
            ...
            <ul className="space-y-3 text-gray-500 text-sm">
              ...
              <li><a href="#fitur" className="hover:text-[#8477e4] transition-colors">Features</a></li>
          ...
          <div className="flex flex-col items-center">
            ...
            <ul className="space-y-3 text-gray-500 text-sm">
              <li><a href="#tim" className="hover:text-[#8477e4] transition-colors">Our Team</a></li>
              <li><a href="#faq" className="hover:text-[#8477e4] transition-colors">FAQ</a></li>
              <li><a href="#" className="hover:text-[#8477e4] transition-colors">Blog</a></li>
            </ul>
          </div>
        </div>
